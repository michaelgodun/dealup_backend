class Api::V1::AuthController < Api::V1::ApiBaseController
  skip_before_action :authenticate_token, only: [:create, :register]

  def create
    user = User.find_by(email: params[:email])
    if user&.valid_password?(params[:password])
      access_token = JsonWebToken.encode({ sub: user.id }, 5.minutes.from_now)
      refresh_token = JsonWebToken.encode({ sub: user.id }, 7.days.from_now)
      user.update!(refresh_token: refresh_token)
      render json: { user: { id: user.id, username: user.username, email: user.email }, access_token: access_token, refresh_token: refresh_token }, status: :ok
    else
